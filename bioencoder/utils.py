...
def groupencode(sequence):
    AA_top = 'NDCEGHILMFTWYV-'
    AA_down = 'ARKPQS'
    code = []
    for aa in sequence.upper() :
        site = AA_down.find(aa)
    return code
def split_seq(seq,window_size=13):
    if(len(seq)<(window_size*2+1)):
        print("Seq length less than window size")
        return
    c_pos = int((len(seq)-1)/2)
    left_seq = seq[:c_pos]
    right_seq = seq[c_pos+1:]
    return left_seq[-window_size:]+"C"+right_seq[:window_size]